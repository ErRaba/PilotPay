...
(function(window) {
  ...
  /**
   * E3b: flag de envío de analytics (default OFF).
   * El envío a Firebase está pausado hasta que pilotpay/analytics tenga reglas
   * (Fase 1.3+). El tracking local, el buffer y la API pública NO cambian.
   */
  ...
  function uploadBatch(events) {
    // E3b: pausa de envío (default OFF) → sin fbUpdate, sin console.error.
    // flush() llama a uploadBatch, por lo que hereda la pausa sin tocarse.
